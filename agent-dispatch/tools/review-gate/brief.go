...
// reviewer could actually open. A relative path is resolved against the repo
// root the brief prints ONLY, matching the root the reviewer is told to use.
// That makes a target that exists only in the caller's cwd fail closed instead
// of passing validation for a file absent from the declared checkout.
func targetExists(root, p string) bool {
	...
	if !filepath.IsAbs(p) && root != "" {
		cands = []string{filepath.Join(root, p)}
	}
	for _, c := range cands {
