...
You are **read-only** in outcome: no commits, and no lasting changes to the tree. The one sanctioned exception is the **mutation check** (step 3): you may temporarily break/revert the code under test via shell edits, and you MUST restore the tree to its exact pre-check state (`git stash` / `git checkout`) before returning. Your returned message IS the review.
